...
//
// 校验随包的 dsh 应用是否撑得住它对外宣称的 bundle：面板分区靠两个硬编码名单（客户端
// BUILTIN_PROFILE_BUNDLES / 宿主 OPTIONAL_BUNDLES），而 profile 的 dsh.profile.bundles 与
// 「是否随包」解耦 —— 选中不等于安装，面板开关也只写 bundles；我们的便携包是 pnpm deploy 的
// 子集，兜不兜得住必须验，否则只会在启动时行加载失败。
//
// 名单不写死：从已部署的包里解析，上游改名/新增会自动覆盖；解析不出来 → 退出码 2（逼人来看，
// 不静默放过）。
//
// 用法：
